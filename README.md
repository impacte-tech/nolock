...
- **Native Notebooks (`.ipynb`)** — Colab-style Jupyter notebooks rendered natively: markdown cells with `$…$` math, auto-height Monaco code cells, rich outputs (images, HTML, tables, **typeset LaTeX via KaTeX**), and a **persistent Python kernel per notebook** running inside a **Python virtual environment** you pick (or create) from the toolbar. State survives across cells; Shift+Enter runs and advances. Notebook **magic commands** work too — `%pip install …` installs into the running kernel's venv with live output, plus `!shell` escape, `%who`/`%whos`, and `%%bash` cells. **⬇ Export** produces a self-contained printable HTML (open it anywhere → Ctrl+P → PDF), and the `.ipynb` uploads directly into Google Colab with outputs and math preserved.
